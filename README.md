...
- **Bank details are intentionally not stored in `invoice-template.xlsx`**,
  since this repo is public. The **Bank Details** section on the form is
  optional — leave it blank to omit that section from the invoice, or fill
  it in per-invoice if you want it printed. Nothing is saved between visits;
  you'd re-enter it each time (or fork the repo privately and hardcode it
  into the template if you'd rather it always appear).
